...
<xsl:template name="search_results_body">
  ...
  <!-- *** Top separation bar *** -->
<!-- XXX: Disable top separation bar so we see only results
    <xsl:if test="Q != ''">
      <xsl:call-template name="top_sep_bar">
...
-->
    ...
    <!-- *** Google footer *** -->
<!-- XXX: Remove copyright so we see only results
    <xsl:call-template name="copyright"/>
-->
...
<select name="site">
 ...
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='eng')]">
 ...
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='flmnh')]">
   <option value="flmnh" selected="selected">flmnh</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="flmnh">flmnh</option>
  </xsl:otherwise>
 </xsl:choose>
</select>
...
<!-- XXX: Remove paging so we see only one page of results
    <xsl:call-template name="google_navigation">
      <xsl:with-param name="prev" select="RES/NB/PU"/>
      <xsl:with-param name="next" select="RES/NB/NU"/>
      <xsl:with-param name="view_begin" select="RES/@SN"/>
  ...
  <table cellpadding="0" cellspacing="0" border="0">
    <tr>
      <td class="s">
        ...
        <xsl:if test="HN">
<!-- XXX: Hide 'more results' links to keep survey blind
          <br/>
          [
